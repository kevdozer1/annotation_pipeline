...
def _write_summary(results: pd.DataFrame, path: Path) -> None:
    ...
    perceptive_delta = (float(grouped["perceptive"]) - baseline) / baseline * 100.0
    text = (
        "# Benchmark Summary\n\n"
        f"The Mode A proxy grid contains {len(results)} runs: 4 annotation families x 1 scale x 3 seeds. "
        f"Hybrid is the best-scoring family in this POC table ({grouped[best]:.6f} latent MSE), "
        f"and perceptive supervision beats baseline by {abs(perceptive_delta):.1f}% at 13 episodes. "
